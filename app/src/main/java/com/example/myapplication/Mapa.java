...
import com.example.myapplication.databinding.ActivityMapaBinding;
...
public class Mapa extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        LatLng lima = new LatLng(-31, 150);
        mMap.addMarker(new MarkerOptions().position(lima).title("Marker in Lima"));
        mMap.moveCamera(CameraUpdateFactory.newLatLng(lima));

        LatLng comas = new LatLng(-34, 149);
        mMap.addMarker(new MarkerOptions().position(comas).title("Marker in Lima"));
